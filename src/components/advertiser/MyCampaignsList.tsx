...
export const MyCampaignsList = () => {
  ...
  const totalRevenue = mockCampaigns.reduce((acc, c) => acc + parseInt(c.totalRevenue.replace(/\D/g, "")), 0);
  const totalCommissions = mockCampaigns.reduce((acc, c) => acc + parseInt(c.totalCommissions.replace(/\D/g, "")), 0);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-white dark:bg-zinc-900/50 p-6 md:p-8 rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-sm">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-zinc-900 dark:text-white tracking-tight">My Campaigns</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Active", value: String(mockCampaigns.filter((c) => c.status === "Active").length), icon: Play },
          { label: "Total Revenue", value: `$${totalRevenue.toLocaleString()}`, icon: TrendingUp },
          { label: "Total Commissions", value: `$${totalCommissions.toLocaleString()}`, icon: BarChart3 },
        ].map((s, i) => {
          const Icon = s.icon;
          return (
            <Card key={i} className="bg-white dark:bg-zinc-900/80 border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-sm">
              <CardContent className="p-5">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">{s.label}</p>
                    <p className="text-xl font-extrabold text-zinc-900 dark:text-white mt-1">{s.value}</p>
                  </div>
      ...
      {filtered.length === 0 ? (
        <Card className="border border-slate-200 dark:border-zinc-800 shadow-sm rounded-2xl bg-white dark:bg-zinc-900/50">
          <CardContent className="py-20 text-center">
            ...
            <h3 className="text-xl font-bold text-zinc-900 dark:text-white mb-2">No campaigns found</h3>
            <p className="text-zinc-500 dark:text-zinc-400 mb-8">Try adjusting your search or filter.</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filtered.map((campaign) => {
            ...
            return (
              <Card key={campaign.id} className="group border border-slate-200 dark:border-zinc-800 shadow-sm bg-white dark:bg-zinc-900/80 rounded-3xl hover:shadow-md transition-all">
                <CardHeader className="pt-6 px-6 pb-4 border-b border-slate-100 dark:border-zinc-800/50">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-lg font-bold text-zinc-900 dark:text-white leading-tight">{campaign.name}</CardTitle>
                        <div className="font-mono text-[10px] text-zinc-400 dark:text-zinc-500 mt-0.5">{campaign.contractAddress}</div>
                      </div>
                    </div>
                    <span className={`inline-flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-bold px-3 py-1.5 rounded-full shrink-0 ${cfg.badge}`}>
                ...
                <CardContent className="px-6 pt-5 pb-6">
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    {[
                      ...
                      const Icon = m.icon;
                      return (
                        <div key={i} className="bg-slate-50 dark:bg-zinc-950/50 rounded-xl p-3 text-center border border-slate-100 dark:border-zinc-800/50">
                          <Icon className="w-4 h-4 text-zinc-400 dark:text-zinc-500 mx-auto mb-1" />
                          <div className="font-extrabold text-sm text-zinc-900 dark:text-white">{m.value}</div>
                          <div className="text-[10px] font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">{m.label}</div>
                        </div>
                      );
                    })}
                  </div>
